...
class TestV3OrgsUsers:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_retrieve(self, client: Ngc) -> None:
        v3_orgs_user = client.v3_orgs_users.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        )
        assert_matches_type(User, v3_orgs_user, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: Ngc) -> None:
        response = client.v3_orgs_users.with_raw_response.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        )
        ...
        v3_orgs_user = response.parse()
        assert_matches_type(User, v3_orgs_user, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: Ngc) -> None:
        with client.v3_orgs_users.with_streaming_response.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        ) as response:
    ...
    def test_path_params_retrieve(self, client: Ngc) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_name` but received ''"):
            client.v3_orgs_users.with_raw_response.retrieve(
                user_email_or_id="user-email-or-id",
                org_name="",
            )
...
class TestAsyncV3OrgsUsers:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncNgc) -> None:
        v3_orgs_user = await async_client.v3_orgs_users.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        )
        assert_matches_type(User, v3_orgs_user, path=["response"])
    ...
    async def test_raw_response_retrieve(self, async_client: AsyncNgc) -> None:
        response = await async_client.v3_orgs_users.with_raw_response.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        )
        ...
        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        v3_orgs_user = await response.parse()
        assert_matches_type(User, v3_orgs_user, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncNgc) -> None:
        async with async_client.v3_orgs_users.with_streaming_response.retrieve(
            user_email_or_id="user-email-or-id",
            org_name="org-name",
        ) as response:
            assert not response.is_closed
    ...
    async def test_path_params_retrieve(self, async_client: AsyncNgc) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_name` but received ''"):
            await async_client.v3_orgs_users.with_raw_response.retrieve(
                user_email_or_id="user-email-or-id",
                org_name="",
            )
